...
export default function Blerje() {
  ...
  const fetchData = async() => {
    try{
      ...
      showToast(t('Gabim gjate komunikimit me server') ,'error');
    }finally{
      setLoading(false)
    }
  }
  ...
  const handleRegjistro = async () => {
    ...
    if (!authData.perdoruesiID || !menyraPagesesID) {
      setLoading(false); 
      return showToast(t('Ju Lutem Plotesoni te Gjitha Fushat!'), 'warn');
    }
    const data = {
      totaliPerPagese,
      totaliPageses,
    ...
    try {
      ...
      if (result.success) {
        navigate("/faqjaKryesore", { state: { showToast: true,message: t('Blerja u Regjistrua me Sukses !'),type: 'success' } });
      } else {
    ...
    <Container fluid className="pt-5">
      <Row className="border-bottom"> 
        <Col>
          <Form.Group as={Row} controlId="subjekti" className="mb-2 ">
            <Form.Label column xs={6} className="text-start w-auto">{t('Furnitori')}:</Form.Label>
          ...
          <Form.Group as={Row} controlId="kontakti" className="mb-2">
            <Form.Label column xs={6} className="text-start w-auto">{t('Kontakti')}:</Form.Label>
            <Col xs={6} className="w-75">
        ...
        <Col md={2}>
          <Form.Group>
            <Form.Label column xs={6} className="text-start w-auto">{t('Nr i Fatures')}:</Form.Label>
            <Form.Control type="text" value={nrFatures} onChange={ (e) => kontrolloValidetin(e.target.value)}/>
          </Form.Group>
        </Col>
        <Col md={2}>
          <Form.Group>
            <Form.Label column xs={6} className="text-start w-auto">{t('Data Fatures')}:</Form.Label>
            <Form.Control type="date" value={dataEFatures} onChange={handleDataFatures}/>
          </Form.Group>
        </Col>
        <Col className="d-flex justify-content-center align-items-center">
          <Button variant="info" className="p-2 fs-5" onClick={() => navigate('/blerjet')}>{t('Te Gjitha Blerjet')}</Button>
        </Col>
        
      </Row>
      {nukPranohetNrFatures ? <p className="text-center text-danger">{t('Numri i Fatures Egziston tek ky Subjekt, Nuk Lejohet!')}</p> : null}
            ...
            <Table striped bordered hover size="sm" className="text-center">
              <thead>
                <tr className="fs-5">
                  ...
                  <th scope="col">{t("Cmimi Blerjes Per Cope")}</th>
                  <th scope="col">{t("Sasia Aktuale")}</th>
                  <th scope="col">{t("Sasia e Blerjes")}</th>
      ...
      <Row className="border-top mt-2">
        <Col className="d-flex flex-row justify-content-center align-items-center">
            <Form.Group>
              <Form.Label>{t('Fature e Rregullt')}</Form.Label>
        ...
        <Col xs={12} md={6} className="d-flex justify-content-center mt-3">
          <Form.Control as="textarea" onChange={handleKomentiBlerjesChange} rows={3} className="p-3" placeholder={t("Shkruaj komentin...")} />
        </Col>
      </Row>
      ...
      <Row className="section3 my-5 d-flex justify-content-end">
        <Col xs={12} md={6} className="d-flex justify-content-center align-items-end">
          <Button variant="danger" size="lg" className="mx-2 fs-1" onClick={handleAnulo}>{t('Anulo')}</Button>
          ...
          <div className="d-flex flex-column w-100 justify-content-end">
            ...
            {meFatureTeRregullt?<>
              <Form.Group as={Row} controlId="totaliTvsh" className="mb-2">
                <Form.Label column xs={6} className="text-end">{t('Totali i TVSH')}:</Form.Label>
                <Col xs={6}>
                  <InputGroup >
                    <Form.Control
                      type="text"
