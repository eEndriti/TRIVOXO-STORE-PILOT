...
export default function NdryshoBlerjen() {
   ...
   setInputDisabled(true)
    ...
    try{
      await window.api.ndryshoBlerje(dataPerNdryshim)
        navigate('/faqjaKryesore/' , {state:{showToast:true , message:t('Blerja u ndryshua me sukses!') , type:'success'}})
    }catch(e){
      showToast(t('Gabim gjate ndryshimit te blerjes!'), 'error');
    }finally{
  ...
  return (
    ...
    <Container fluid className='mt-4'>
         ...
         <Row>
            <h4 className='text-center text-secondary fw-bold'>{t('Ndryshimi i Blerjes')} : {blerjeID}</h4>
        </Row>
        <hr/>
                    ...
                    <Table striped bordered hover size="sm" className="text-center">
                      <thead>
                        <tr className="fs-5">
                          ...
                          <th scope="col">{t("Purchase Price per Unit")}</th>
                          <th scope="col">{t("Current Quantity")}</th>
                          <th scope="col">{t("Purchased Quantity")}</th>
...
<br/><br/>
        ...
        <Col className="d-flex flex-row justify-content-center align-items-center">
            <Form.Group>
              <Alert variant='danger'>
                <FontAwesomeIcon icon={faWarning} className='mx-2'/>{data.fatureERregullt ? t('Blerja eshte me Fature te Rregullt!') : t('Blerja nuk eshte me Fature te Rregullt!')}
            </Alert>
            </Form.Group>
        </Col>
        <Col xs={12} md={6} className="d-flex justify-content-center">
          <Form.Control as="textarea" onChange={handleDataChange} name='komentiBlerjes' rows={3} className="p-3" placeholder={t("Shkruaj komentin...")} value={data.komentiBlerjes}/>
        </Col>
      </Row>
      ...
      <Row className="section3 my-5 d-flex justify-content-end">
        <Col xs={12} md={6} className="d-flex justify-content-center align-items-end">
            <Button variant="danger" size="lg" className="mx-2 fs-1" onClick={() => navigate('/faqjaKryesore/')} disabled={inputDisabled}>{t('Anulo')}</Button>
