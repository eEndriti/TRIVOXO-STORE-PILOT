...
export default function Shitje() {
  ...
  const handleTotaliPagesesChange = (e) => {
    ...
    if (value <= totaliPerPagese) {
      ...
      showToast(t('Shuma e paguar nuk mund të jetë më e madhe se totali!'),'error');
    }
  };
  ...
  const handleRegjistro = async () => {
    ...
    try {
      ...
      if (result.success) {
        ...
        message = t('Shitja u regjistrua me sukses!')
        ...
        message = t('Gabim gjate regjistrimit') + result.error
      }}
    } catch (error) {
      ...
      <Row className="d-flex flex-row justify-content-between">
            <Col className="w-100">
              <Form.Group as={Row} controlId="subjekti" className="mb-2  ">
                <Form.Label column xs={6} className="text-start w-auto">{t('Subjekti')}:</Form.Label>
        ...
        <Col className=" d-flex flex-row justify-content-end">
          <Button variant="info" className="text-dark border fs-5 p-4 m-2" onClick={() => navigate('/shitjet')}>{t('Te Gjitha Shitjet')}</Button>
        </Col>
      </Row>
