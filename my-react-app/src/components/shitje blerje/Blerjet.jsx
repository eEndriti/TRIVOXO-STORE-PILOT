...
export default function Blerjet() {
    ...
    const fetchData = async () => {
        try {
            ...
            showToast(t('Gabim gjate shkarkimit te te dhenave') + e.message, 'error');
        }finally{
            setLoading(false)
        }
    }
    ...
    const handleDeleteBlerje = async () => {
        ...
        try {
            ...
            showToast(t('Blerja u Anulua me Sukses !'), 'success');
...
const hapeShtoPagese = (item) =>{
    ...
    return (
        <Container fluid>
            <h4 className="text-center fw-bold pt-4">{t('Të Gjitha Blerjet')}:</h4>
            <hr />
                    ...
                    <Col>
                        {filteredBlerjet.length < 1 ? (
                            <h5 className="text-center text-danger mt-5">
                                {t('Nuk Ekzistojnë Blerje në këtë Interval Kohor!')}
                            </h5>
                        ) : (
                                   ...
                                   <Table responsive bordered striped  hover className="text-center" >
                                        <thead className="table-light">
                                                <tr className="fs-5">
                                                    ...
                                                    <th scope="col">{t("Nderrimi dhe Data e Blerjes")}</th>
                                                    <th scope="col">{t("Data e Fatures")}</th>
                                                    <th scope="col">{t("Fature e Rregullt")}</th>
                                                    <th scope="col">{t("Nr i Fatures")}</th>
                                            ...
                                            <tbody className="">
                                                {filteredBlerjet.slice().reverse().map((item, index) => (
                                                    <tr key={index}>
                                                        ...
                                                        <td>{item.fatureERregullt === 'true' ? t('Po') : t('Jo')}</td>
