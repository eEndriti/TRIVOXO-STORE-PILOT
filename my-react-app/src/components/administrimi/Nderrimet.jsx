...
export default function Nderrimet() {
  ...
  useEffect(() => {
    const fetchData = async () => {
      ...
      try {
        ...
        showToast(t('Gabim gjatë marrjes së të dhënave:') + err, 'error');
      } finally {
        setLoading(false);
      }
    };
  ...
  const mbyllNderrimin = async () => {
    ...
    try {
      ...
      showToast(t('Ndërrimi u mbyll me sukses!'), 'success');
      localStorage.clear();
      Cookies.clear()
      return;
    } catch (err) {
      showToast(t('Gabim gjatë mbylljes:') + err, 'error');
    } finally {
  ...
  const ndryshojeAvansin = async () => {
    ...
    try {
      ...
      showToast(t('Avansi u ndryshua me sukses!'), 'success');
      Cookies.set('avansi', nderrimiAktiv.avansi);
    } catch (err) {
      showToast(t('Gabim gjatë ndryshimit të avansit') + err, 'error');
    } finally {
      setButtonLoading(false);
  ...
  return (
    <>
      <Container>
        <h2 className="mb-4">{t('Menaxhimi i Ndërrimeve')}</h2>
        ...
        {nderrimiAktiv && (
          <Card className="mb-4 shadow-sm">
            <Card.Body>
              <h5>
                {t('Ndërrimi Aktual')} <Badge bg="success">{t('Aktive')}</Badge>
              </h5>
              <p>
                <strong>{t('Filloi me')}:</strong>{' '}
                { formatUTCDate(nderrimiAktiv.dataFillimit)  }
              </p>
              <p>
                <strong>{t('Avansi')}:</strong>{' '}
        ...
        <Row className="mb-3">
          <Col md={6}>
            <Form.Group controlId="filterStartDate">
              <Form.Label>{t('Data Fillimit')}</Form.Label>
              <Form.Control
                type="date"
                value={dataFillimit}
          ...
          <Col md={6}>
            <Form.Group controlId="filterEndDate">
              <Form.Label>{t('Data Mbarimit')}</Form.Label>
              <Form.Control
                type="date"
                value={dataMbarimit}
          ...
          <Card.Body style={{ maxHeight: '400px', overflowY: 'auto' }}>
            <Table responsive bordered hover>
              <thead className="table-light">
                <tr>
                  <th>{t('Nr.')}</th>
                  <th>{t('Koha Fillimit')}</th>
                  <th>{t('Koha Mbylljes')}</th>
                  <th>{t('Statusi')}</th>
                  <th>{t('Avansi')}</th>
                  <th>{t('Totali pa Avans')}</th>
                </tr>
              </thead>
              <tbody>
                {filteredNderrime.length === 0 && (
                  <tr>
                    <td colSpan={6} className="text-center text-muted">
                      {t('Nuk ka të dhëna për këtë periudhë')}.
                    </td>
                  </tr>
                )}
                {filteredNderrime
                    ...
                    <tr key={item.nderrimiID || idx}>
                      ...
                      <td>
                        <Badge bg={item.iHapur ? 'success' : 'secondary'}>
                          {item.iHapur ? t('Aktive') : t('I Mbyllur')}
                        </Badge>
                      </td>
                      <td>
                        {item.avansi
        ...
        >
          <Modal.Header closeButton>
            <Modal.Title>{t('Ndrysho Avansin e Aktivit')}</Modal.Title>
          </Modal.Header>
          <Modal.Body>
            <Form.Group>
              <Form.Label>{t('Vlera e Avansit')} (€)</Form.Label>
              <InputGroup>
                <Form.Control
                  type="number"
                  min={0}
        ...
        >
          <Modal.Header closeButton>
            <Modal.Title>{t('Konfirmo Mbylljen e Ndërrimit')}</Modal.Title>
          </Modal.Header>
          <Modal.Body>
            <Alert variant="warning" className="d-flex align-items-center">
              ...
              />
              <div>
                <strong>{t('Kujdes!')}</strong> {t('Pas mbylljes do t’ju duhet të kyçeni prapë dhe të startoni ndërrimin.')}
              </div>
            </Alert>
          </Modal.Body>
          <Modal.Footer>
