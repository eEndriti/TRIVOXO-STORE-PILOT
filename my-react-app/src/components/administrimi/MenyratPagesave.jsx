...
export default function MenyratPagesave() {
    ...
    const emptyData = () => {
      ...
      const shtoOpsion = async () => {
        ...
        try {
            ...
            showToast(t('Menyra e Pageses u Regjistrua me Sukses'), 'success');
            ...
            showToast(t('Gabim gjate Regjistrimit te Menyres se Pageses:') + error, 'error');
        } finally {
    ...
    const ndryshoOpsion = async () => {
        ...
        try {
            ...
            showToast(t('Menyra e Pageses u Ndryshua me Sukses'), 'success')
            setTimeout(() => {
                setModal(false);
                setTriggerReload(prev => !prev);
            }, 2500); 
        } catch (error) {
            showToast(t('Gabim gjate Ndryshimit te Menyres se Pageses:') + error, 'error');
        } finally {
            setButtonLoading(false);
    ...
    const handleConfirm = async() => {
        if(dataPerDelete){
          ...
          try{
            ...
            if(result){
                showToast(t('Menyra e Pageses u Anulua me Sukses'),'success')
            setTimeout(() => {
                setTriggerReload(prev => !prev);
            }, 2500); 
            }else{
                showToast(t('Gabim gjate Anulimit se Menyres se Pageses:') + result, 'error');

            }
        } catch (error) {
            showToast(t('Gabim gjate Anulimit se Menyres se Pageses:') + error, 'error');
        } finally {
            setButtonLoading(false);
            fetchData()
        }
      ...
      const handleTransferoMjete = async () => {
        ...
        try{
            ...
            showToast(t('Mjetet u Transferuan me Sukses'),'success')
            setTimeout(() => {
                setLevizjeModal(false)
                setTriggerReload(prev => !prev);
            }, 2500); 
        }catch(error){
            showToast(t('Gabim gjate Transferimit te Mjeteve') + error, 'error');
        }finally{
            setButtonLoading(false)
        }
      }
  ...
  <>
    {loading ? <AnimatedSpinner/> : <Container>
        <Row>
            <Col lg={4} >
                <Form.Control placeholder={t('Kerko me Emertim...')} value={emertimiSearch} onChange={(e) => setEmertimiSearch(e.target.value)} />
            </Col>
        </Row>
        <Row>
            <Table striped bordered hover className="mt-3">
                ...
                <tr key={index}>
                    ...
                    <>  
                        ...
                        <td>{item.asociuarMeArken ? <>{item.emertimi} ({t('Asociuar me Arken!')})</> : item.emertimi }</td>
                        <td>
            ...
            <Row className="mt-3">
                <Col>
                    ...
                    <div className="ms-auto d-flex align-items-center gap-2">
                        ...
                        >
                        {t('Levizje Interne (Depozitim / Tërheqje)')}
                        </Button>
                        <Button
                        ...
                        className="me-2"
                        data-toggle="tooltip"
                        title={t("Shfaq Historikun e Transferimeve")}
                        >
        ...
        {showHistorinTransfereve && <Row className='mt-3'>
            ...
            >
                <Modal.Header closeButton>
                    <Modal.Title className="text-dark">{!perNdryshim ? <>{t('Forma për Regjistrim të Menyres se Pagesave')}</> : <>{t('Forma për Ndryshim të Menyres se Pagesave')}</>}</Modal.Title>
                </Modal.Header>
                <Modal.Body>
                    <Form>
                        <Row className="mb-3">
                            <Col md={6}>
                                <Form.Group controlId="formFirstName">
                                    ...
                                    <Form.Control
                                        ...
                                        placeholder={t("Shkruaj Emertimin...")}
                                    />
                                </Form.Group>
                            </Col>
                            <Col md={6}>
                                <Form.Group controlId="formLastName">
                                    <Form.Label>{t('Shuma Aktuale')}:</Form.Label>
                                    <InputGroup>
                                    <Form.Control
                                        type="text"
                                        name="shuma"
                                        value={data.shuma}
                                        onChange={handleChangeData}
                                        placeholder={t("Shkruaj Shumen Aktuale...")}
                                    />
             ...
             show={levizjeModal} onHide={()=> {buttonLoading ? null :setLevizjeModal(false)}}> 
                ...
                <Modal.Body>
                    ...
                    <Form.Group controlId="neOpsionin">
                        <Form.Label>{t('Transfero në')}:</Form.Label>
                        <Form.Control
                        as="select"
                        ...
                        >
                        <option value="">{t('Selekto Opsionin Perfitues')}:</option>
                            {menyratPagesave
