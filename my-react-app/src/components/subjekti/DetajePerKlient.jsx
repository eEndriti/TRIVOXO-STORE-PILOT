...
import { Container, Row, Col, Form, Spinner, Button, OverlayTrigger, Tooltip, Badge } from 'react-bootstrap';
import { useParams } from 'react-router-dom';
import { FontAwesomeIcon } from '@fortawesome/react-fontawesome';
import { faChevronDown, faChevronRight,faTrashCan,faEye,faEuroSign,faFileExcel } from '@fortawesome/free-solid-svg-icons';
import AuthContext, { formatCurrency, formatLongDateToAlbanian, normalizoDaten } from '../AuthContext';
...
export default function DetajePerKlient() {
    ...
    const [shtoPageseModal,setShtoPageseModal] = useState(false)    
    ...
    const filteredTransaksionet = useMemo(() => {
        ...
        return data
            .filter(item => {
                ...
                return itemDate >= normalizoDaten(startDate) && itemDate <= normalizedEndDate;
            })
...
const hapeShtoPagese = (item) =>{
    ...
    let IDDokumentit = ''
    switch (true) {
        case item.shifra.startsWith('SH-'):
          llojiDokumentit = 'shitje';
            IDDokumentit = item.shitjeID;
          break;
      
        case item.shifra.startsWith('S-'):
            llojiDokumentit = 'servisimi';
            IDDokumentit = item.servisimiID;
          break;
      
        case item.shifra.startsWith('B-'):
            llojiDokumentit = 'blerje';
            IDDokumentit = item.blerjeID;
          break;
      
        default:
          showToast('Gabim', 'error');
      }
      
    setDataPerShtoPagese({
        ...item,
        subjekti:subjekti[0].emertimi,
        llojiDokumentit,
        IDDokumentit
    })
    setShtoPageseModal(true)
}

    const eksportoExcel = async () => {
        let isoFmt = new Intl.DateTimeFormat('sv-SE').format(new Date());

        const fileName = `Transaksionet_${subjekti[0].emertimi}_${isoFmt}.xlsx`;
        
        const cleanedTransaksionet = filteredTransaksionet.map(({
            shitjeID,
            menyraPagesesID,
            perdoruesiID,
            transaksioniID,
          ...
          const summaryData = cleanedTransaksionet.map((row, idx) => {
            let dateToShow = row.dataShitjes || row.dataPerfundimit
            ...
            return {
              ...
              lloji:         row.lloji,
              totaliPerPagese: row.totaliPerPagese,
              totaliPageses: row.totaliPageses,
              mbetjaPerPagese: row.mbetjaPerPagese,
              Data_e_Transaksionit:          dateToShow,
              komenti:       row.komenti
            }
          })

         const cleanedPagesat = pagesat.map(({
      pagesaID,
      transaksioniID: _t,
      subjektiID:     _s,
      menyraPagesesID:_m,
    ...
    const paymentsData = cleanedPagesat.map((row, idx) => ({
        Nr: idx + 1,
      ...row
    }))

        try {
            const result  = await handleExport(fileName, summaryData, paymentsData)
    ...
    return (
                                        ...
                                        {filteredTransaksionet.slice().reverse().map((item, index) => {
                                            ...
                                            return (
                                                <tr key={index}>
                                                    ...
                                                    <td >
                                                        <Badge bg={item.mbetjaPerPagese > 0 ? 'danger' : 'success'} pill className='fw-bold'>{formatCurrency(item.mbetjaPerPagese)}</Badge>
                                                    </td>
                                                    {lloji == 'klient' ?  
                                                    ...
                                                    <td>
                                                            ...
                                                            <FontAwesomeIcon 
                                                                className={`px-3 ${activeShifra === item.shifra ? 'text-primary' : 'text-dark'} 
                                                                `}
                                                                onClick={() => {nrPagesave <= 0 ? null : detajetEPagesave(item)}} 
                                                                icon={activeShifra === item.shifra ? faChevronDown : faChevronRight} 
                                                            />

                                                       
                                                            <Button variant="btn btn-outline-success"  
                                                                onClick={() => hapeShtoPagese(item)} disabled={item.mbetjaPerPagese <= 0 || item.lloji == 'online'}>
        ...
        <ShtoPagese show={shtoPageseModal} handleClose={() => setShtoPageseModal(false)} data={dataPerShtoPagese} />
